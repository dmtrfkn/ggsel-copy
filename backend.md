...
5. транзакция `createTx`: списать промокод (`redeemPromoWithin`) → посчитать `discount` →
   `INSERT orders (... status='created')`. Всё в одной транзакции: лимит не прошёл →
   заказа нет; `INSERT` упал → откат снимает и списание промокода.
...
параллельный выполняет тот же `UPDATE`, но статус уже `delivering` — его нет в списке →
`changes === 0` → выходит. К поставщику идёт **один**.
