...
Этого достаточно, чтобы руками пройти сценарий этапа 3: опустошить пул → купить →
увидеть заказ в списке «оплачен, но не выдан» → пополнить → «Повторить выдачу» →
заказ становится `delivered`.
