...
Ручной сценарий покупки: на витрине нажать **Купить** на любой карточке →
в модалке **Создать заказ** → **Оплатить — успех** → откроется страница статуса,
...
```
created ──paid──▶ paid ──▶ delivering ──▶ delivered        (основной путь)
   │
   └──failed──▶ payment_failed                             (финал)

delivering ──пул пуст──▶ out_of_stock ──пополнение + retry──▶ delivered
delivering ──оба поставщика упали──▶ delivery_failed ──retry──▶ delivered
```
